...
def evaluate_lenet5(learning_rate=0.1, n_epochs=2000, L2_weight=0.00001, max_performance=0.513333333334,rel_emb_size=300, margin1=0.5, margin2=0.5, ent_emb_size=300, batch_size=7, maxPathLen=5, neg_size=4, path_size=10):
...
#     print train_pos_masks_matrix_folders[0]
#     exit(0)
    train_neg=corpus[1]
...
#     load_model_from_file(rootPath+'Best_Paras_v1_0.513333333333', params_to_store)
    ...
    target_rel_reps_tensor3=target_rel_emb.dimshuffle('x','x', 0)

    pos_neg_entities_tensor=pos_neg_entities_tensor.dimshuffle(2, 0, 1)  #( #neg, batch*#p, len)

    pos_paths_input=rel_embeddings[pos_path_id_matrix.flatten()].reshape((pos_path_id_matrix.shape[0],maxPathLen, rel_emb_size)).dimshuffle(0,2,1) # (batch, hidden, len)
    pos_ensemble_path_input=pos_paths_input+pos_path_w2v_tensor3.dimshuffle(0,2,1)  #(batch, hidden ,len)
    pos_init_heads_input=ent_embeddings[pos_init_heads].reshape((pos_path_id_matrix.shape[0], ent_emb_size))
    pos_ground_truth_entities=ent_embeddings[pos_target_entities.flatten()].reshape((pos_path_id_matrix.shape[0], maxPathLen, ent_emb_size)).dimshuffle(0,2,1)# (batch, hidden, len)
    pos_neg_entities=ent_embeddings[pos_neg_entities_tensor.flatten()].reshape((neg_size, pos_path_id_matrix.shape[0], maxPathLen, ent_emb_size)).dimshuffle(0,1,3,2) #(#neg, batch, hidden ,len)

    #GRU
    pos_gru_input = pos_ensemble_path_input   #gru requires input (batch_size, emb_size, maxSentLen)
    pos_gru_layer=GRU_Batch_Tensor_Input_with_Mask_with_MatrixInit(pos_gru_input, pos_path_mask,  pos_init_heads_input, ent_emb_size, U1, W1, b1)
    pos_pred_ent_embs=pos_gru_layer.output_tensor  # (batch_size, hidden_size, len)
    ...
    pos_loss_seq2seq=T.sum(pos_raw_loss.flatten()[pos_valid_indice_list])
    ...
    pos_batch_cosine=T.nnet.sigmoid(T.log(T.sum(T.exp(pos_cos_rel2path), axis=1))) # (batch, 1))
    ...
    neg_gru_layer=GRU_Batch_Tensor_Input_with_Mask_with_MatrixInit(neg_gru_input, neg_path_mask,  neg_init_heads_input, ent_emb_size, U1, W1, b1)
    ...
    neg_loss_seq2seq=T.sum(neg_raw_loss.flatten()[neg_valid_indice_list])
    ...
    neg_batch_cosine=T.nnet.sigmoid(T.log(T.sum(T.exp(neg_cos_rel2path), axis=1)))
    ...
    rank_loss=T.sum(T.maximum(0.0, margin2+neg_batch_cosine_matrix-pos_batch_cosine_matrix))


    loss=pos_loss_seq2seq+neg_loss_seq2seq+rank_loss
...
#     updates = opt.updates(params, grads, np.float32(0.01) / np.cast['float32'](batch_size), np.float32(0.9))
    ...
    train_model = theano.function([pos_init_heads,pos_path_id_matrix,pos_path_w2v_tensor3,pos_path_mask,pos_target_entities,pos_neg_entities_tensor,
                                   neg_init_heads,neg_path_id_matrix,neg_path_w2v_tensor3,neg_path_mask,neg_target_entities,neg_neg_entities_tensor,
                                   target_rel_id,target_rel_w2v_emb], cost, updates=updates,on_unused_input='ignore')

    test_model = theano.function([pos_init_heads,pos_path_id_matrix,pos_path_w2v_tensor3,pos_path_mask,#pos_target_entities,pos_neg_entities_tensor,
                                target_rel_id,target_rel_w2v_emb], pos_batch_cosine, on_unused_input='ignore')
...
#                     print 'ent_idmatrix_pos[:,0]', ent_idmatrix_pos[:,0]
                    cost_i+=train_model(ent_idmatrix_pos[:,0],
                                                   rel_idmatrix_pos,
                    ...
                    if iter%100==0:
                        ...
                        for folder_id in range(folder_size):
                            ...
                            for test_pos_iter in test_batch_start_pos:
                                    ...
                                    cosine_batch_pos=test_model(ent_idmatrix_pos[:,0],
                                                                   rel_idmatrix_pos,
                                                                   np.asarray(rel_idmatrix_to_word2vec_init(rel_idmatrix_pos, rel_id2wordlist, word2vec, 300), dtype=theano.config.floatX),
                                                                   mask_matrix_pos,
                                                                   target_rel_id,
                            ...
                            for   test_neg_iter in test_batch_start_neg:             
                                ...
                                cosine_batch_neg=test_model(ent_idmatrix_neg[:,0],
                                                               rel_idmatrix_neg,
                                                               np.asarray(rel_idmatrix_to_word2vec_init(rel_idmatrix_neg, rel_id2wordlist, word2vec, 300), dtype=theano.config.floatX),
                                                               mask_matrix_neg,
                                                               target_rel_id,
                            ...
                            map=compute_map(cosine_pos_list+cosine_neg_list, [1]*test_pos_pair_size+[0]*test_neg_pair_size)
                            print '\t\t\t temp map:', map
                            aver_map+=map
