...
from preprocess import load_word2vec_to_init_rels, rel_idmatrix_to_word2vec_init, rel_idlist_to_word2vec_init, load_das_v2, compute_map, rel_id_to_word2vec_init
...
def evaluate_lenet5(learning_rate=0.1, n_epochs=2000, L2_weight=0.00001, max_performance=75.0,rel_emb_size=200, margin1=0.5, margin2=0.5, ent_emb_size=200, batch_size=20, maxPathLen=8, neg_size=4, path_size=30):
...
#     load_model_from_file(rootPath+'Best_Paras_v1_0.513333333333', params_to_store)
    ...
    target_rel_emb=rel_embeddings[target_rel_id]#+rel_w2v_embeddings[target_rel_id]
...
#     pos_path_w2v_tensor3=rel_w2v_embeddings[pos_path_id_matrix.flatten()].reshape((pos_path_id_matrix.shape[0],maxPathLen, rel_emb_size)).dimshuffle(0,2,1)
    
    
    
    pos_ensemble_path_input=pos_paths_input#+pos_path_w2v_tensor3  #(batch, hidden ,len)
    ...
    pos_gru_layer2=GRU_Batch_Tensor_Input_with_Mask_with_MatrixInit(pos_gru_input, pos_path_mask,  pos_init_heads_input, ent_emb_size, U2, W2, b2)
    pos_pred_ent_embs=pos_gru_layer2.output_tensor  # (batch_size*#p, hidden_size, len)
...
#     neg_path_w2v_tensor3=rel_w2v_embeddings[neg_path_id_matrix.flatten()].reshape((neg_path_id_matrix.shape[0],maxPathLen, rel_emb_size)).dimshuffle(0,2,1)
    
    neg_ensemble_path_input=neg_paths_input#+neg_path_w2v_tensor3 #(batch, hidden ,len)
    ...
    neg_gru_layer2=GRU_Batch_Tensor_Input_with_Mask_with_MatrixInit(neg_gru_input, neg_path_mask, neg_init_heads_input, ent_emb_size, U2, W2, b2)
    neg_pred_ent_embs=neg_gru_layer2.output_tensor  # (batch_size, hidden_size, len)
    ...
    loss=(pos_loss_seq2seq+neg_loss_seq2seq)*0.1+rank_loss


    params = [rel_embeddings, ent_embeddings]+NN_para +NN_para2  # put all model parameters together
...
#     updates = opt.updates(params, grads, np.float32(0.01) / np.cast['float32'](batch_size), np.float32(0.9))
    ...
    train_model = theano.function([pos_init_heads,pos_path_id_matrix,pos_path_mask,pos_target_entities,pos_neg_entities_tensor,pos_pathPad_mask,
                                   neg_init_heads,neg_path_id_matrix,neg_path_mask,neg_target_entities,neg_neg_entities_tensor,neg_pathPad_mask,
                                   target_rel_id], cost, updates=updates,on_unused_input='ignore')

    test_model = theano.function([pos_init_heads,pos_path_id_matrix,pos_path_mask,pos_pathPad_mask,#pos_target_entities,pos_neg_entities_tensor,
