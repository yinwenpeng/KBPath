...
#     NN_para2=[U2, W2, b2]     #U1 includes 3 matrices, W1 also includes 3 matrices b1 is bias
    ...
    U2, W2, b2=create_GRU_para_U3W4b3(rng, rel_emb_size, ent_emb_size)
    NN_para2=[U2, W2, b2]     #U1 includes 3 matrices, W1 also includes 3 matrices b1 is bias
...
#     paths_input_w2v=rel_w2v_embeddings[path_id_matrix.flatten()].reshape((batch_size,maxSentLen, rel_emb_size)).dimshuffle(0,2,1) # (batch, hidden, len)
    ...
    ground_truth_entities=ent_embeddings[target_entities.flatten()].reshape((batch_size, maxSentLen, ent_emb_size)).dimshuffle(0,2,1)# (batch, hidden, len)
    neg_entities=ent_embeddings[neg_entities_tensor.flatten()].reshape((neg_size, batch_size, maxSentLen, ent_emb_size)).dimshuffle(0,1,3,2) #(#neg, batch, hidden ,len)
...
#     pred_last_ents=gru_layer.output_sent_rep + init_heads_input #(batch, hidden)
    ...
    ground_truth_entities_as_init=T.concatenate([init_heads_input.dimshuffle(0,1,'x'), ground_truth_entities[:,:,:-1]], axis=2) #(batch, hidden ,len)
    gru_layer2=GRU_OneStep_MatrixInit_Tensor3Init_Tensor3Input(gru_layer.output_tensor, init_heads_input, ground_truth_entities_as_init, ent_emb_size, U2, W2, b2)   # (batch_size, hidden_size, len)
...
#     loss=T.sum(raw_loss*path_mask.dimshuffle('x', 0,1))
    ...
    loss=T.mean(raw_loss.flatten()[valid_indice_list])
...
#     updates = opt.updates(params, grads, np.float32(0.01) / np.cast['float32'](batch_size), np.float32(0.9))
    ...
    test_model = theano.function([init_heads,path_id_matrix, path_mask, target_entities], cosines_test_matrix, on_unused_input='ignore')
...
#                                       np.asarray(rel_idmatrix_to_word2vec_init(test_rel_idmatrix, rel_id2wordlist, word2vec, 300), dtype=theano.config.floatX),
                ...
                if hit10 > max_acc:
                    ...
                    if max_acc > max_performance:
                        store_model_to_file(rootPath+'Best_Paras_arc3_task1_gru_twoinit_'+str(max_acc), params)
