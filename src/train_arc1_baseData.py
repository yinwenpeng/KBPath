...
#     path_w2v_tensor3=T.ftensor3('path_w2v_tensor3') #(batch, len, emb_size)
    ...
    target_entities=T.ivector()  #batch
    neg_entities_matrix=T.imatrix() #in the beginning (batch, #neg)
...
#     neg_entities_tensor=neg_entities_tensor.dimshuffle(2, 0, 1)
    ...
    ground_truth_entities=ent_embeddings[target_entities.flatten()].reshape((batch_size,ent_emb_size))# (batch, hidden)
    neg_entities=ent_embeddings[neg_entities_matrix.flatten()].reshape((batch_size, neg_size, ent_emb_size)) #( batch, #neg, hidden)
...
#     pred_ent_embs=gru_layer.output_sent_rep  # (batch_size, hidden_size)
    ...
    simi_negs=cosine_tensor3_tensor4(pred_last_ents.dimshuffle(0,1,'x'), neg_entities.dimshuffle(1,0,2,'x')) #(#neg, batch)  inputs: (batch, hidden, len)), (#neg, batch, hidden, len)
    raw_loss=T.maximum(0.0, margin+simi_negs-simi_grounds.dimshuffle('x',0))
    loss=T.sum(raw_loss)
...
#     updates = opt.updates(params, grads, np.float32(0.01) / np.cast['float32'](batch_size), np.float32(0.9))
    ...
    train_model = theano.function([init_heads,path_id_matrix, path_mask, target_entities, neg_entities_matrix], cost, updates=updates,on_unused_input='ignore')
...
#                                       np.asarray(rel_idmatrix_to_word2vec_init(test_rel_idmatrix, rel_id2wordlist, word2vec, 300), dtype=theano.config.floatX),
                ...
                if hit10 > max_acc:
                    ...
                    if max_acc > max_performance:
                        store_model_to_file(rootPath+'Best_Paras_arc1_baseData_'+str(max_acc), params_to_store)
